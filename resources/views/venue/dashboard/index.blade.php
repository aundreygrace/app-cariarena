...
<section class="cards">
    <div class="card">
        ...
        <h3>{{ $todayBookingsCount }}</h3>
        <small>{{ $todayBookingsCount > 0 ? '+12% dari kemarin' : 'Belum ada booking hari ini' }}</small>
    </div>
    <div class="card">
        <small>Pendapatan Minggu Ini</small>
        <h3>Rp {{ number_format($weeklyRevenue, 0, ',', '.') }}</h3>
        <small>{{ $weeklyRevenue > 0 ? '+8% dari minggu lalu' : 'Belum ada pendapatan' }}</small>
    </div>
    <div class="card">
        <small>Rating Rata-rata</small>
        <h3>{{ $averageRating }}</h3>
        <small>dari {{ $totalReviews }} review</small>
    </div>
    <div class="card">
        <small>Tingkat Okupansi</small>
        <h3>{{ $occupancyRate }}%</h3>
        <small>Bulan {{ \Carbon\Carbon::now()->translatedFormat('F') }}</small>
    </div>
</section>
    ...
    <div class="booking">
        ...
        <div class="booking-item">
            <div class="booking-info">
                <strong>{{ $booking->nama_customer }}</strong>
                <p>
                    @if($booking->venue)
                        {{ $booking->venue->name }} — 
                    @else
                        Venue — 
                    @endif
                    {{ \Carbon\Carbon::parse($booking->waktu_booking)->format('H:i') }} - 
                    {{ \Carbon\Carbon::parse($booking->end_time)->format('H:i') }} 
                    ({{ $booking->durasi }} jam)
                </p>
                <small>{{ \Carbon\Carbon::parse($booking->tanggal_booking)->translatedFormat('d F Y') }}</small>
            </div>
            <div>
                <strong>Rp{{ number_format($booking->total_biaya, 0, ',', '.') }}</strong>
                <div class="status {{ $booking->status == 'Terkonfirmasi' || $booking->status == 'Selesai' ? 'confirmed' : 'pending' }}">
                    {{ $booking->status }}
                </div>
            </div>
        </div>
                            ...
                            <div class="form-section">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="facilities-grid">
                                        @foreach(['parking', 'toilet', 'canteen', 'ac', 'prayer_room', 'locker_room', 'waiting_room', 'sound_system'] as $facility)
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" value="{{ $facility }}" id="check{{ ucfirst(str_replace('_', '', $facility)) }}" name="facilities[]">
                                            <label class="form-check-label" for="check{{ ucfirst(str_replace('_', '', $facility)) }}">
                                                @php
                                                    $facilityNames = [
                                                ...
                                                @endphp
                                                {{ $facilityNames[$facility] ?? ucwords(str_replace('_', ' ', $facility)) }}
                                            </label>
                                        </div>
                                        @endforeach
                                    </div>
