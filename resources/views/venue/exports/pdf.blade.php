...
<body>
    ...
    <p>
    <strong>
        {{ ($isAdmin ?? false) ? 'Admin:' : 'Pemilik Venue:' }}
    </strong>
    {{ $user->name }}
</p>
        <p><strong>Email:</strong> {{ $user->email }}</p>
        <p><strong>Periode:</strong> {{ $periode }}</p>
        <p><strong>Tanggal Export:</strong> {{ $tanggalExport }}</p>
    </div>
    ...
    <div class="charts-row mb-4">
        ...
        <div class="chart-section">
            ...
            <div class="chart-container">
                <img class="chart-img" src="{{ $revenueChartImg }}">
            </div>
        </div>
        ...
        <div class="chart-section">
            ...
            <div class="chart-container">
                <img class="chart-img pie" src="{{ $venueChartImg }}">
            </div>
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $no++ }}</td>
                <td>{{ $row->transaction_number ?? $row->kode }}</td>
                <td>{{ $row->pengguna ?? $row->customer }}</td>
                <td>{{ $row->nama_venue ?? $row->venue }}</td>
                <td>{{ $row->metode_pembayaran ?? $row->metode }}</td>
                <td class="text-right">Rp {{ number_format($row->amount ?? $row->jumlah, 0, ',', '.') }}</td>
                <td>
                    {{ \Carbon\Carbon::parse($row->transaction_date ?? $row->tanggal)->format('d/m/Y') }}
                </td>
                <td>{{ $row->durasi }} jam</td>
                <td>
                    @if(($row->status ?? '') === 'completed' || ($row->status ?? '') === 'Selesai')
                        <span class="badge badge-success">Selesai</span>
                    @elseif(($row->status ?? '') === 'pending' || ($row->status ?? '') === 'Pending')
                        <span class="badge badge-warning">Pending</span>
                    @elseif(($row->status ?? '') === 'cancelled' || ($row->status ?? '') === 'Dibatalkan')
                        <span class="badge badge-danger">Dibatalkan</span>
                    @elseif(($row->status ?? '') === 'refunded' || ($row->status ?? '') === 'Dikembalikan')
