...
<script>
    ...
    function initializeTimeSlots() {
        ...
        timeSlots.forEach(time => {
            ...
            timeSlot.innerHTML = `
                ...
                <div class="price">Rp ${currentVenue.price_per_hour.toLocaleString('id-ID')}/jam</div>
            `;
            
            timeSlot.addEventListener('click', function() {
                if (this.classList.contains('booked')) {
                    alert('Waktu ini sudah dipesan. Silakan pilih waktu lain.');
                    return;
                }
        ...
        const firstAvailableSlot = timeGrid.querySelector('.time-slot:not(.booked)');
    ...
    function isDurationConflict(dateString, startTime, duration) {
        ...
        for (let i = 0; i < duration; i++) {
            ...
            const timeKey = `${dateString}_${checkTime}`;
    ...
    function updateSummary() {
        ...
        const selectedTimeSlot = document.querySelector('.time-slot.selected');
        if (selectedTimeSlot) {
            const time = selectedTimeSlot.querySelector('.time').textContent;
    ...
    function proceedToPayment() {
        ...
        const formattedDate = `${dayNames[selectedDate.getDay()]}, ${selectedDate.getDate()} ${monthNames[selectedDate.getMonth()]} ${selectedDate.getFullYear()}`;
        
        const totalBiaya = currentVenue.price_per_hour * hours;
        
        const paymentUrl = "{{ route('pesan.bayar') }}" + 
            "?date=" + encodeURIComponent(formattedDate) + 
            "&time=" + encodeURIComponent(time) + 
            "&duration=" + encodeURIComponent(hours + ' Jam') +
            "&hours=" + hours +
            "&total=" + totalBiaya +
            "&venue_id=" + currentVenue.id +
            "&venue_name=" + encodeURIComponent(currentVenue.name) +
            "&address=" + encodeURIComponent(currentVenue.address) +
            "&venue_price=" + currentVenue.price_per_hour;
