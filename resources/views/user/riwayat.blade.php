        ...
        <div class="order-grid">
            @if(isset($orders) && count($orders) > 0)
                @foreach($orders as $order)
                    @php
                    // Ambil status sebenarnya dari database
                    $actualStatus = strtolower($order->status ?? 'menunggu');
    
                    // Konversi status untuk tampilan
                    // Jika status di database adalah 'lunas', tampilkan sebagai 'Selesai'
                    if ($actualStatus === 'lunas' || $actualStatus === 'selesai' || $actualStatus === 'completed') {
                        $statusClass = 'badge-selesai';
                        $statusText = 'Selesai';
                        $filterStatus = 'selesai';
                    } elseif ($actualStatus === 'dibatalkan' || $actualStatus === 'canceled') {
                        $statusClass = 'badge-dibatalkan';
                        $statusText = 'Dibatalkan';
                        $filterStatus = 'dibatalkan';
                    } elseif ($actualStatus === 'pending' || $actualStatus === 'menunggu') {
                        ...
                        $statusClass = 'badge-menunggu';
                        $statusText = ucfirst($actualStatus);
                        $filterStatus = 'menunggu';
                    }
                    
                    // Cek apakah sudah ada rating
                    // Perbaikan: Ambil dari relasi atau field yang sesuai
                    $hasRating = false;
                    $rating = 0;
                    $reviewComment = '';
                    ...
                    if (isset($order->review) && !empty($order->review)) {
                        $hasRating = true;
                        $reviewComment = $order->review;
                        $rating = $order->rating ?? 0;
                    } elseif (isset($order->reviews) && $order->reviews->count() > 0) {
                        // Jika menggunakan relasi Laravel
                        $hasRating = true;
                        $reviewComment = $order->reviews->first()->comment ?? '';
                        $rating = $order->reviews->first()->rating ?? 0;
                    }
                    
                    // Tentukan apakah bisa memberi rating
                    // Bisa rating jika status SELESAI dan belum ada rating
                    $canRate = ($statusClass === 'badge-selesai' && !$hasRating);
                    
                    // Tentukan apakah bisa membatalkan
                    // Bisa batal jika status MENUNGGU (belum dikonfirmasi/bayar)
                    $canCancel = ($statusClass === 'badge-menunggu');
                    
                    // Format data lainnya
                    // Pastikan data tersedia dengan nilai default
                    $dateFormatted = $order->tanggal ?? $order->date_formatted ?? date('d M Y');
                    $timeRange = $order->waktu ?? $order->time_range ?? '10:00';
                    $totalPrice = $order->total_harga ?? $order->total_biaya ?? 0;
                    $formattedPrice = number_format($totalPrice, 0, ',', '.');
                    $venueName = $order->venue->name ?? $order->venue_name ?? 'Venue';
                    $location = $order->venue->address ?? $order->location ?? 'Alamat tidak tersedia';
                    $shortLocation = (strlen($location) > 60) ? substr($location, 0, 57) . '...' : $location;
                    $venueId = $order->venue_id ?? $order->venue->id ?? 0;
                    $bookingCode = $order->kode_booking ?? $order->booking_code ?? '';
                    $orderId = $order->id ?? 0;
                    $category = $order->venue->category ?? $order->category ?? 'Badminton';
                    
                    // Dapatkan gambar venue (sama seperti sebelumnya)
                    $venueImage = '';
                    if (isset($order->venue) && isset($order->venue->photo)) {
                        $venueImage = $order->venue->photo;
                    } elseif (isset($order->venue_gambar) && !empty($order->venue_gambar)) {
                        $venueImage = $order->venue_gambar;
                    } else {
                        $defaultImages = [
                            ...
                            'basket' => 'https://images.unsplash.com/photo-1546519638-68e109498ffc?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80',
                            'tenis' => 'https://images.unsplash.com/photo-1595435934247-5d33b7f92c70?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80',
                            'voli' => 'https://images.unsplash.com/photo-1592656094267-764a603b45e0?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80',
                        ];
                        ...
                        if (empty($venueImage)) {
                            $venueImage = $defaultImages['badminton'];
                        }
                    }
                    
                    if (!empty($venueImage) && !filter_var($venueImage, FILTER_VALIDATE_URL)) {
                        $venueImage = asset('storage/' . $venueImage);
                    }
                @endphp
                    ...
                    @if($statusClass === 'badge-selesai')
                        @if($hasRating)
                            <!-- TAMPILAN JIKA SUDAH ADA RATING -->
                        ...
                        @else
                            <!-- TOMBOL BERI RATING JIKA BELUM ADA RATING -->
                            <div class="action-section">
                                <button class="action-btn btn-rating" onclick="openRatingModal(
                                    {{ $orderId }}, 
                                    '{{ $bookingCode }}', 
                        ...
                        @endif
                    @elseif($statusClass === 'badge-menunggu')
                        @if($canCancel)
                    ...
                    @else
                        ...
                        <div class="rating-section">
                            ...
                            <div class="no-rating">- Tidak tersedia -</div>
                        </div>
                    @endif
                    </div>
                @endforeach
    ...
    <div class="modal-container-cancel">
        ...
        <div class="cancel-modal-body">
            ...
            <div class="refund-details">
                ...
                <div class="refund-row">
                    <div class="refund-label">Biaya Admin</div>
                    <div class="refund-value">Rp 5.000</div>
                </div>
...
function submitRating() {
    ...
    const venueId = document.getElementById('ratingVenueId').value;
    const reviewText = document.getElementById('reviewText').value.trim();
    
    // Kirim data ke server - PERBAIKAN: Gunakan URL langsung
    fetch('/riwayat/submit-review', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            alert(data.message);
            // Refresh halaman untuk menampilkan rating baru
