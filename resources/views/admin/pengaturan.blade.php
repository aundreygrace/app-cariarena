...
@section('content')
    ...
    <div class="horizontal-nav">
        ...
        <a href="#" class="{{ session('activeSection') == 'pengaturan-sistem' ? 'active' : '' }}" data-target="pengaturan-sistem">Pengaturan Sistem</a>
        <a href="#" class="{{ session('activeSection') == 'manajemen-admin' ? 'active' : '' }}" data-target="manajemen-admin">Manajemen Admin</a>
        <a href="#" class="{{ session('activeSection') == 'pengaturan-notifikasi' ? 'active' : '' }}" data-target="pengaturan-notifikasi">Notifikasi</a>
        <a href="#" class="{{ session('activeSection') == 'keamanan-backup' ? 'active' : '' }}" data-target="keamanan-backup">Keamanan & Backup</a>
    ...
    <div class="settings-content">
        ...
        <section id="profil-akun" class="settings-section {{ session('activeSection', 'profil-akun') == 'profil-akun' ? 'active' : '' }}">
            ...
            <form method="POST" action="{{ route('admin.pengaturan.profile.update') }}" enctype="multipart/form-data" id="profileForm">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="role" value="{{ ucfirst(auth()->user()->roles->first()->name ?? 'Admin') }}" readonly disabled>
        ...
        <section id="pengaturan-sistem" class="settings-section {{ session('activeSection') == 'pengaturan-sistem' ? 'active' : '' }}">
            ...
            <form method="POST" action="{{ route('admin.pengaturan.system.update') }}" id="systemForm">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="nama-aplikasi" name="app_name" placeholder="Masukkan nama aplikasi" 
                           value="{{ old('app_name', config('app.name', 'CariArena')) }}" required
                           class="@error('app_name') is-invalid @enderror">
                    @error('app_name')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="email" id="email-aplikasi" name="app_email" placeholder="Masukkan email aplikasi" 
                           value="{{ old('app_email', config('mail.from.address', 'noreply@cariarena.com')) }}" required
                           class="@error('app_email') is-invalid @enderror">
                    @error('app_email')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="bahasa" name="language" class="@error('language') is-invalid @enderror">
                        <option value="id" {{ old('language', config('app.locale', 'id')) == 'id' ? 'selected' : '' }}>Bahasa Indonesia</option>
                ...
                <div class="form-group">
                    ...
                    <select id="zona-waktu" name="timezone" class="@error('timezone') is-invalid @enderror">
                        <option value="Asia/Jakarta" {{ old('timezone', config('app.timezone', 'Asia/Jakarta')) == 'Asia/Jakarta' ? 'selected' : '' }}>WIB (UTC+7)</option>
                        <option value="Asia/Makassar" {{ old('timezone', config('app.timezone')) == 'Asia/Makassar' ? 'selected' : '' }}>WITA (UTC+8)</option>
                        <option value="Asia/Jayapura" {{ old('timezone', config('app.timezone')) == 'Asia/Jayapura' ? 'selected' : '' }}>WIT (UTC+9)</option>
                    </select>
                    @error('timezone')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <select id="format-tanggal" name="date_format" class="@error('date_format') is-invalid @enderror">
                        <option value="d/m/Y" {{ old('date_format', 'd/m/Y') == 'd/m/Y' ? 'selected' : '' }}>DD/MM/YYYY</option>
                        <option value="m/d/Y" {{ old('date_format', 'd/m/Y') == 'm/d/Y' ? 'selected' : '' }}>MM/DD/YYYY</option>
                        <option value="Y-m-d" {{ old('date_format', 'd/m/Y') == 'Y-m-d' ? 'selected' : '' }}>YYYY-MM-DD</option>
                    </select>
                    @error('date_format')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
        ...
        <section id="manajemen-admin" class="settings-section {{ session('activeSection') == 'manajemen-admin' ? 'active' : '' }}">
            ...
            <!-- Form Tambah Admin -->
            <form method="POST" action="{{ route('admin.pengaturan.admin.add') }}" id="formTambahAdmin">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select id="role-admin" name="role" class="@error('role') is-invalid @enderror">
            ...
            <div class="table-container">
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span id="admin-status-{{ $admin->id }}" class="badge {{ $admin->email_verified_at ? 'badge-success' : 'badge-danger' }}">
                                        {{ $admin->email_verified_at ? 'Aktif' : 'Nonaktif' }}
                                    </span>
                                </td>
                                ...
                                <td>
                                    <div class="btn-group btn-group-sm">
                                        <button class="btn btn-info edit-admin" 
                                                ...
                                                data-role="{{ $roleName }}"
                                                data-status="{{ $admin->email_verified_at ? 'aktif' : 'nonaktif' }}">
                    ...
                    <div class="mt-3 text-muted">
                        <small>Total Admin: {{ $admins->count() }}</small>
                    </div>
                    @endif
                </div>
            </div>
        ...
        <section id="pengaturan-notifikasi" class="settings-section {{ session('activeSection') == 'pengaturan-notifikasi' ? 'active' : '' }}">
            ...
            <form method="POST" action="{{ route('admin.pengaturan.notifications.update') }}" id="notificationForm">
                @csrf
                    ...
                    <div class="checkbox-group">
                        ...
                        <div class="checkbox-item">
                            <input type="checkbox" id="email-report" name="email_report" 
                                   {{ old('email_report', session('notification_settings.email_report', true)) ? 'checked' : '' }}>
                        ...
                        <div class="checkbox-item">
                            <input type="checkbox" id="email-venue" name="email_venue" 
                                   {{ old('email_venue', session('notification_settings.email_venue', true)) ? 'checked' : '' }}>
                        ...
                        <div class="checkbox-item">
                            <input type="checkbox" id="email-support" name="email_support" 
                                   {{ old('email_support', session('notification_settings.email_support', true)) ? 'checked' : '' }}>
        ...
        <section id="keamanan-backup" class="settings-section {{ session('activeSection') == 'keamanan-backup' ? 'active' : '' }}">
            ...
            <form method="POST" action="{{ route('admin.pengaturan.security.update') }}" id="securityForm">
                @csrf
                ...
                <div class="backup-settings">
                    ...
                    <div class="form-group">
                        ...
                        <select id="frekuensi-backup" name="backup_frequency" class="@error('backup_frequency') is-invalid @enderror">
                            <option value="daily" {{ old('backup_frequency', 'weekly') == 'daily' ? 'selected' : '' }}>Harian</option>
                            <option value="weekly" {{ old('backup_frequency', 'weekly') == 'weekly' ? 'selected' : '' }} selected>Mingguan</option>
                            <option value="monthly" {{ old('backup_frequency', 'weekly') == 'monthly' ? 'selected' : '' }}>Bulanan</option>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="jumlah-backup" name="backup_retention" 
                               value="{{ old('backup_retention', 30) }}" min="1" max="365"
                               class="@error('backup_retention') is-invalid @enderror">
...
<div class="modal fade" id="editAdminModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="formEditAdmin" action="{{ route('admin.pengaturan.admin.update') }}" method="POST">
                @csrf
...
<div class="modal fade" id="hapusAdminModal" tabindex="-1" aria-labelledby="hapusAdminModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{ route('admin.pengaturan.admin.delete') }}" id="formHapusAdmin">
                @csrf
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.resetProfilForm = function() {
            if (confirm('Reset semua perubahan? Data akan dikembalikan ke nilai semula.')) {
                ...
                if (form) {
                    ...
                    const previewContainer = document.getElementById('photoPreview');
                    const userHasPhoto = @json($hasPhoto);
        ...
        window.createBackup = function() {
            if (confirm('Apakah Anda yakin ingin membuat backup sekarang?')) {
                ...
                fetch("{{ route('admin.pengaturan.backup.now') }}", {
                    method: 'POST',
                    headers: {
