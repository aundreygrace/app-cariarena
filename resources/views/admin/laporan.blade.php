...
@section('content')
    ...
    <div class="cards">
        ...
        <div class="card">
            ...
            <small>{{ $jumlahPemesananVenue ?? 0 }}x dipesan bulan ini</small>
        </div>
    </div>
    ...
    <div class="filter-card">
        ...
        <div class="section-body">
            <form method="GET" action="{{ route('admin.laporan.index') }}" id="filterForm" class="filter-form">
                <div class="row g-2 align-items-end">
                    <div class="col-md">
                        <label class="form-label">Periode</label>
                        <select class="form-select" id="periode" name="periode">
                            <option value="hari-ini" {{ request('periode') == 'hari-ini' ? 'selected' : '' }}>Hari Ini</option>
                            <option value="minggu-ini" {{ request('periode', 'minggu-ini') == 'minggu-ini' ? 'selected' : '' }}>Minggu Ini</option>
                    ...
                    <div class="col-md">
                        ...
                        <select class="form-select" id="jenisLaporan" name="jenis_laporan">
                            <option value="semua" {{ request('jenis_laporan', 'semua') == 'semua' ? 'selected' : '' }}>Semua Laporan</option>
                            <option value="pemesanan" {{ request('jenis_laporan') == 'pemesanan' ? 'selected' : '' }}>Pemesanan</option>
                            <option value="pendapatan" {{ request('jenis_laporan') == 'pendapatan' ? 'selected' : '' }}>Pendapatan</option>
                            <option value="venue" {{ request('jenis_laporan') == 'venue' ? 'selected' : '' }}>Venue</option>
                            <option value="pengguna" {{ request('jenis_laporan') == 'pengguna' ? 'selected' : '' }}>Pengguna</option>
                        </select>
                    </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover" id="laporanTable">
                    ...
                    <tbody id="laporanTableBody">
                        ...
                        <tr>
                            <td class="fw-bold">{{ $transaction->transaction_number ?? 'N/A' }}</td>
                            <td>{{ $transaction->pengguna ?? 'N/A' }}</td>
                            <td>{{ $transaction->nama_venue ?? 'N/A' }}</td>
                            ...
                            <td class="d-none-mobile">{{ $transaction->durasi ?? '2' }} jam</td>
                            ...
                            <td>
                                ...
                                <span class="badge {{ $badgeClass }}" title="Status: {{ $rawStatus }}">
                                    {{ $displayStatus }}
                                </span>
                            </td>
                            <td>
                                <div class="btn-group btn-group-sm">
                                    <button class="btn btn-eye" title="Lihat Detail" data-bs-toggle="modal" data-bs-target="#detailLaporanModal" data-pemesanan-id="{{ $transaction->id }}">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const venueDistribution = @json($venueDistribution);
        ...
        function updateChartData(period) {
            showLoading();
            fetch(`{{ route('admin.laporan.chart-data') }}?period=${period}`, {
                headers: {
